...
class SaleService
{
        ...
        return DB::transaction(function () use ($data) {
            ...
            foreach ($data['items'] as $item) {
                ...
                if ($availableStock < $item['quantity']) {
                    throw new BusinessException("Stok {$product->name} di rak display tidak mencukupi. Sisa: {$availableStock}");
                }
            ...
            foreach ($itemsToCreate as $item) {
                ...
                // Stok sumber kebenaran: product_stocks di lokasi display.
                // Jika row belum ada, treat sebagai 0 (produk belum pernah distok di rak).
                $before = $productStock ? (int) $productStock->stock : 0;
                if ($productStock) {
                    $productStock->decrement('stock', $item['quantity']);
                }

                // Sinkronkan mirror products.stock dari sum(product_stocks)
                $this->inventoryService->syncStockMirror($product->id);
    ...
    public function cancel(Sale $sale, string $reason = ''): Sale
    {
        return DB::transaction(function () use ($sale, $reason) {
            ...
            // Re-lock sale di dalam transaction untuk cegah cancel ganda
            ...
            foreach ($sale->items as $item) {
                ...
                // Update or create product stock at display location (lock dulu di query)
                $productStock = ProductStock::where('product_id', $product->id)
                    ->where('location_id', $displayLocation->id)
                    ->lockForUpdate()
                    ->first();
            ...
            // Batalkan hutang pelanggan jika transaksi berstatus 'debt'
            if ($originalStatus === 'debt' && $sale->customer_id) {
                $debt = CustomerDebt::where('sale_id', $sale->id)
                ...
                if ($debt) {
                    ...
                    $debt->update([
                        'status' => 'paid',
                        'remaining_amount' => 0,
                        'notes' => trim('Dibatalkan: '.($reason ?: $sale->invoice_number)),
                    ]);
                    Customer::where('id', $sale->customer_id)->decrement('current_debt', $remaining);
                }
            }
