...
import { getCustomerDebts, payCustomerDebt } from '@/api'
...
export default function CustomerDebts() {
  ...
  const [payDebt, setPayDebt] = useState<CustomerDebt | null>(null)
  const [payForm, setPayForm] = useState({ amount: '', payment_method: 'cash', notes: '' })

  const { data, isLoading } = useQuery({
    queryKey: ['customer-debts', page, status],
    queryFn: () => getCustomerDebts({ page, status: status || undefined, per_page: 20 }).then(r => r.data),
  })

  const payMut = useMutation({
    mutationFn: (d: object) => payCustomerDebt(payDebt!.id, d),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['customer-debts'] })
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? <Spinner /> : (
        <div className="card p-0">
          <div className="tbl-wrapper">
            <table className="tbl">
              <thead>
                <tr><th>Pelanggan</th><th>Invoice</th><th>Total Hutang</th><th>Terbayar</th><th>Sisa</th><th>Jatuh Tempo</th><th>Status</th><th>Aksi</th></tr>
              </thead>
              <tbody>
                {data?.data.map((d) => (
                  <tr key={d.id}>
                    <td className="font-medium">{d.customer?.name}</td>
                    <td><span className="font-mono text-xs">{d.sale?.invoice_number ?? '-'}</span></td>
                ...
                {data?.data.length === 0 && <tr><td colSpan={8} className="text-center py-12 text-slate-400">Tidak ada data hutang</td></tr>}
              </tbody>
            </table>
          </div>
      ...
      <Modal open={!!payDebt} onClose={() => setPayDebt(null)} title="Bayar Hutang Pelanggan" size="sm">
        {payDebt && (
          <div className="space-y-4">
            <div className="bg-slate-50 rounded-xl p-3 text-sm">
              <p className="font-semibold">{payDebt.customer?.name}</p>
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button onClick={() => payMut.mutate({ amount: Number(payForm.amount), payment_method: payForm.payment_method, notes: payForm.notes })}
                disabled={payMut.isPending} className="btn-primary">{payMut.isPending ? 'Memproses...' : 'Catat Pembayaran'}</button>
            </div>
          </div>
        )}
      </Modal>
